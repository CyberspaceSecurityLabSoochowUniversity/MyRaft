...
func candidateLoop(s *server,conn *net.UDPConn) {
	fmt.Println("CandidateLoop Start")
	//fmt.Println(s.peers)
	s.vote = 1	//自己的一票
	//timeoutChan := time.Tick(afterBetween1(s.ElectionTimeout(), s.ElectionTimeout()*2))
	...
	go func() {
		for s.State() == Candidate{
			...
			if s.State() != Candidate{
				if data1.Id == HeartBeatOrder || data1.Id == VoteBackOrder{
					break
				}
			}
